...
out = cv.VideoWriter('../resources/outputCollorSpace.avi',cv.VideoWriter_fourcc('M','J','P','G'), 
30, (width, height))
...
while (mov.isOpened()):
    ...
    if ret == True:
        ...
        circles = cv.HoughCircles(filter_gray, cv.HOUGH_GRADIENT, 1, 100, param1=30, param2=20, minRadius=50, maxRadius=0)
        ...
        showFrame = frame
        canny2 = cv.Canny(canny,20,70)
        #cv.drawContours(showFrame, contours_c, -1, (0,255,0), 3)
        if circles is not None:
            circles = np.uint16(np.around(circles))
            for i in circles[0,:]:
                cv.circle(showFrame, (i[0], i[1]), i[2], (0,225,0), 2)
